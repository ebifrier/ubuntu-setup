...
./create-lxc.sh 203 app     igoshogi --vlan 20 --ip dhcp   # VLAN 20 に置く
...
### ネットワーク — IP は「第4オクテット = ctid」

IP は既定で **ホストと同じ /24 の第4オクテットに ctid をそのまま使った静的 IP**
になる (`--ip auto`)。サブネットとゲートウェイはホスト自身のブリッジ (`vmbr0`)
から導出するので、設定ファイルは要らない。

| ctid | IP (ホストが 192.168.10.0/24 のとき) | MAC |
|---|---|---|
| 110 | `192.168.10.110` | `BC:24:11:00:01:10` |
| 152 | `192.168.10.152` | `BC:24:11:00:01:52` |
| 200 | `192.168.10.200` | `BC:24:11:00:02:00` |

ctid はクラスタ全体で一意なので、IP の一意性は ctid に任せられる。
**空き IP の帳簿は要らない — 空き ctid がそのまま空き IP で、`pct list` が帳簿になる。**
同じ ctid で作り直しても IP は変わらない。運用ルールは2つだけ:

- ルーターの DHCP 配布範囲をコンテナ用レンジ (`.100`-`.254`) と被らない所に
  しておく (例: `.2`-`.99` を配る)
- クラスタ化するまでは、ホストごとに ctid のレンジを分けて重複を防ぐ
  (例: host1 は 100-149、host2 は 150-199)。クラスタ化後は Proxmox が一意性を
  強制するので、レンジは「どのホスト生まれか」の目印として残るだけ

作成後に Pi-hole の **Local DNS records** へ `hostname → IP` を1行足せば名前でも
引ける (登録すべき内容は完了メッセージに出る)。作成前に同じ IP から応答が
あった場合は止まるので、クラスタ化前にホスト間で ctid が重複してもそこで気付ける。

ctid が 254 を超えるときや別サブネットに置くときは `--ip` を明示する
(`--ip 10.0.20.152/24,gw=10.0.20.1` の形)。

### DHCP に寄せる場合 (--ip dhcp)

MAC は ctid から機械的に決まる。`BC:24:11` は Proxmox の OUI で、残り3バイトに
ctid の10進6桁をそのまま埋める (152 → `BC:24:11:00:01:52`)。MAC を見れば ctid が
分かり、同じ ctid で作り直しても同じ MAC になるので、DHCP の予約さえ書いておけば
作り直しても IP が変わらない。
...
`--vlan <id>` で `net0` に `tag=` が付く。VLAN 先のサブネットはホストのブリッジ
からは分からないので IP の自動採番は使えず、`--ip` の明示が必要 (`dhcp` も可)。
ブリッジが VLAN aware (`vlan_filtering=1`) でなければ PVE が `vmbr0v<id>` を
作って渡すので動きはするが、上流ポートが trunk になっている必要がある。
警告を出すだけで止めはしない。
